...
When changing file, folder or view, change viewind to -1 to make sure it starts
on firstview image.
...
For Python3:
    ...
    From PyQt5 import QtCore, QtGui, Qtmenuitems and change QtGui to
    Qtmenuitems in several places.
    Scale image display.
...
Tidied up the defs which choose view style and left/right so they scroll
through images more neatly.
Separate def for loading preferences.
Revised preferences file.
Rearranged options on GUI into two columns and added progress panel and image
title.
...
prefdir = homedir + '.popout3d/'       # folder for preference file
workdir = homedir + '.popout3d/work/'  # folder for images being worked on
...
spawntext = 'spawn.txt'                # name of output file from spawned prog
...
def getPreferences():
    global version, mydir, myfile, myext, form, formlet, style, stylelet
    global stylecode, view
        ...
        for newfile in os.listdir(mydir):
            if (newfile[0:2] == '3D'
              ...
              or newfile.endswith('.png') or newfile.endswith('.PNG'))):
                ...
                if (view == 'Set' # only add file to viewlist if it's selected
                  and myfile == newfile[2:len(myfile)+2] and myext == newext[1:]):
                    viewlist.append([newfile[2:], newext[1:]])
                elif view == 'Folder': #only add to viewlist if in selectfolder
                    viewlist.append([newfile[2:], newext[1:]])
...
def viewImage(self, fn):
    ...
    # gtk *************************************************************
    #V1.4.2
    #self.labelImage1.set_text('3D'+viewlist[viewind][0]+'.'+viewlist[viewind][1])
    #V1.4.2
    #pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(mydir+'3D'+viewlist[viewind][0]+'.'+viewlist[viewind][1], 900, 550, True) # width, height, preserve_active_ratio
...
def findImage(self): #V1.4.2
    ...
    # provided viewing list isn't empty, select the currently indicated image
    # from the viewlist #V1.4.2 and show it.
    if len(viewlist) > 0:
...
def processPair(self, newfile, newext, leftn, rightn):
    ...
    # images match on type and size align them,
    # else warn they are incompatible and skip them
    if imageLF == imageRF and imageLS == imageRS:

        # if there are no lock files, call the alignment program
        # Quotes around filenames to deal with special characters like "("
...
def processSet(self, newfile, newext):
    ...
    # if there are at least two images in the set, repeatedly call processPair
    # to process them, else warning statusmessage
    if imagestodo == 0:
...
class MyWindowClass(Qtmenuitems.QMainWindow):
    def __init__(self, parent = None):
        ...
        # Bind the event handlers to the radio buttons, no need for Quit 
...
class GUI:
    def __init__(self):
        ...
        #V1.4.2
        # self.labelStatusbar = self.builder.get_object('labelStatusbar') 
        ...
        '''
        # gtk *****************************************************************
        # activating any of these triggers the one activated and the
        # one de-activated
    ...
    def imagemenuitemFolder(self, menuitem):
        ...
        if newdir[0:len(homedir)] == homedir:
            # filename is original name less ending digit
            mydir = newdir; os.chdir(mydir)
        ...
        if newfile[0:len(homedir)] == homedir:
            ...
            if newfile[0:2] != '3D':
                ...
                if newfile[len(newfile)-1:] in ['0','1','2','3','3','5','6','7','8','9']:
                    # filename is original name less ending digit
                    mydir = newdir +'/' ; myfile = newfile[:-1]; myext = newext[1:] 
                else:
    ...
    def buttonFolder(self, menuitem):
        ...
        if todolist != []:
            ...
            for newfile in todolist:
                # filename style is name less last digit, no 3D at beginning
            ...
            if os.path.isfile(workdir+spawnlock):
                ...
                with open(workdir+spawntext, 'w') as f: # output to text file
                    # run spawnprog
                    subprocess.Popen([progdir+spawnprog, workdir, mydir], stdout = f)
        else:
    ...
    def buttonSet(self, menuitem):
        ...
        if myfile != '{none}' and myext != '{none}':
            ...
            # filename style is name less last digit - no 3D at the beginning
            processSet(self, myfile, myext)
